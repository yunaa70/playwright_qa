// 04. 결제 — 배송정보 입력 + 주문 완료. 미입력 차단도 검증.
...
test.describe('04. 결제', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // 공통 준비: 상품 담고 장바구니 → 결제 진입
    await inventory.addFirstProductToCart();
    await inventory.goToCart();
    await cart.checkout();
  });

  test('[TC-CHECK-001] 배송정보 미입력 시 다음으로 진행되지 않는다', async () => {
  ...
  test('[TC-CHECK-002] 배송정보 입력 후 주문 검토 화면으로 이동한다', async () => {
  ...
  test('[TC-CHECK-003] 전체 결제 플로우가 완료된다', async () => {
    await checkout.enterInfo(CHECKOUT.valid.firstName, CHECKOUT.valid.lastName, CHECKOUT.valid.zip);
    await checkout.continue();
